...
const SEASONAL = [
  { type: 'natal',     name: 'Natal & Réveillon', emoji: '🎄', color: '#DC2626', month: 12, day: 25, leadDays: 60, theme: { bg: '#FEF2F2', border: '#FECACA', accent: '#DC2626' } },
  { type: 'maes',      name: 'Dia das Mães',       emoji: '🌸', color: '#DB2777', month: 5,  day: 11, leadDays: 45, theme: { bg: '#FDF2F8', border: '#FBCFE8', accent: '#DB2777' } },
  { type: 'namorados', name: 'Dia dos Namorados',  emoji: '💕', color: '#E11D48', month: 6,  day: 12, leadDays: 45, theme: { bg: '#FFF1F2', border: '#FECDD3', accent: '#E11D48' } },
  { type: 'pascoa',    name: 'Páscoa',             emoji: '🐣', color: '#D97706', month: 4,  day: 20, leadDays: 30, theme: { bg: '#FFFBEB', border: '#FDE68A', accent: '#D97706' } },
  { type: 'carnaval',  name: 'Carnaval',           emoji: '🎭', color: '#7C3AED', month: 2,  day: 28, leadDays: 30, theme: { bg: '#F5F3FF', border: '#DDD6FE', accent: '#7C3AED' } },
  { type: 'pais',      name: 'Dia dos Pais',       emoji: '👔', color: '#0369A1', month: 8,  day: 11, leadDays: 30, theme: { bg: '#EFF6FF', border: '#BFDBFE', accent: '#0369A1' } },
]
...
const STATUS_TEXT: Record<string, string> = { producao: '#92400E', aprovado: '#065F46', publicado: '#065F46', aguardando_aprovacao: '#9D174D', revisao_interna: '#5B21B6', agendado: '#1E40AF' }
...
export default function CampanhasPage() {
  ...
  if (loading) return <div className="p-6 text-sm text-[#A8A59E]">Carregando campanhas...</div>

  return (
    <div className="p-6 flex flex-col gap-6 h-full overflow-y-auto">
      {/* Header */}
      <div>
        <h1 className="text-[#1A1916] font-semibold text-lg">Campanhas</h1>
        <p className="text-[#6B6963] text-sm mt-0.5">Visão global · todos os clientes por campanha</p>
      </div>
          ...
          return (
            <button
              ...
              style={selected === s.type
                ? { background: s.theme.bg, borderColor: s.theme.border, color: s.theme.accent, fontWeight: 500 }
                : { background: 'white', borderColor: '#EBEAE5', color: '#6B6963' }
              }
            >
              <span>{s.emoji}</span>
              <span>{s.name}</span>
              {activeCnt > 0 && <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full" style={{ background: selected === s.type ? s.theme.accent : '#F2F0EB', color: selected === s.type ? 'white' : '#6B6963' }}>{activeCnt}</span>}
      ...
      <div className="rounded-2xl p-5 border" style={{ background: seasonal.theme.bg, borderColor: seasonal.theme.border }}>
      ...
      {activeClients.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-[#1A1916] mb-3">Clientes com esta campanha</p>
          <div className="grid grid-cols-2 gap-4">
            {activeClients.map(client => {
              ...
              return (
                <div key={client.id} className="bg-white border border-[#EBEAE5] rounded-2xl overflow-hidden">
                  {/* Client header */}
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[#1A1916] truncate">{client.name}</p>
                      <p className="text-xs text-[#6B6963]">{campPosts.length} posts · {extras.length} extras</p>
                    </div>
                    {/* Progress */}
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {campPosts.length > 0 && (
                        <div className="flex items-center gap-1.5">
                          <div className="w-16 h-1.5 bg-[#F2F0EB] rounded-full overflow-hidden">
                          ...
                          <span className="text-[10px] text-[#A8A59E]">{donePosts}/{campPosts.length}</span>
                        </div>
                      )}
                      {isExp ? <ChevronUp size={14} className="text-[#A8A59E]" /> : <ChevronDown size={14} className="text-[#A8A59E]" />}
                    </div>
                  </div>
                  ...
                  {isExp && (
                    <div className="border-t border-[#EBEAE5] p-4 flex flex-col gap-4">
                      ...
                      {campPosts.length > 0 && (
                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-[#A8A59E] mb-2">Posts</p>
                          <div className="flex flex-col gap-1.5">
                            {campPosts.map(p => (
                              <div key={p.id} className="flex items-center gap-2 text-xs">
                                <span className="font-bold text-[#A8A59E] w-5">#{p.post_number}</span>
                                ...
                                <span className="flex-1 text-[#1A1916] truncate">{p.title}</span>
                                <span className="text-[10px] font-medium px-1.5 py-0.5 rounded flex-shrink-0" style={{ background: STATUS_BG[p.status] || '#F3F4F6', color: STATUS_TEXT[p.status] || '#374151' }}>{STATUS_LABEL[p.status] || p.status}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {/* Extras */}
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-[#A8A59E] mb-2">Extras {extras.length > 0 && `· ${doneExtras}/${extras.length}`}</p>
                        {extras.length > 0 && (
                          <div className="w-full h-1 bg-[#F2F0EB] rounded-full mb-2 overflow-hidden">
                        ...
                        <div className="flex flex-col gap-1">
                          {extras.map((e: any) => (
                            <div key={e.id} className="flex items-center gap-2">
                              <button onClick={() => toggleExtra(camp.id, e.id, e.done)} className="w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 transition-colors" style={e.done ? { background: seasonal.theme.accent, borderColor: seasonal.theme.accent } : { borderColor: '#C8C5BE' }}>
                        ...
                        {addingExtra === camp.id ? (
                          <div className="flex gap-2 mt-2">
                            <input autoFocus value={newExtraText[camp.id] || ''} onChange={e => setNewExtraText(t => ({ ...t, [camp.id]: e.target.value }))} onKeyDown={e => e.key === 'Enter' && addExtra(camp.id)} placeholder="Novo extra..." className="flex-1 text-xs border border-[#EBEAE5] rounded-lg px-2.5 py-1.5 outline-none focus:border-[#1A1916]" />
                            ...
                            <button onClick={() => setAddingExtra(null)} className="text-xs text-[#A8A59E] px-1">×</button>
                          </div>
                        ) : (
                          <button onClick={() => setAddingExtra(camp.id)} className="flex items-center gap-1 text-xs text-[#A8A59E] hover:text-[#1A1916] mt-1.5 transition-colors"><Plus size={11} /> Extra</button>
                        )}
                      </div>
                      ...
                      {camp.briefing && (
                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-[#A8A59E] mb-1.5">Briefing</p>
                          <p className="text-xs text-[#6B6963] leading-relaxed bg-[#FAFAF8] rounded-lg px-3 py-2">{camp.briefing}</p>
                        </div>
                      )}
      ...
      {inactiveClients.length > 0 && (
        <div>
          <p className="text-sm font-semibold text-[#A8A59E] mb-3">{inactiveClients.length} clientes sem esta campanha</p>
          <div className="flex flex-wrap gap-2">
            {inactiveClients.map(client => (
              <a key={client.id} href={`/dashboard/clientes/${client.id}?tab=campanhas`} className="flex items-center gap-2 bg-white border border-[#EBEAE5] rounded-lg px-3 py-1.5 hover:border-[#D4D1CB] transition-colors">
                ...
                <span className="text-xs text-[#6B6963]">{client.name}</span>
              </a>
            ))}
          </div>
        </div>
      ...
      {activeClients.length === 0 && (
        <div className="flex flex-col items-center justify-center h-48 text-center border border-dashed border-[#EBEAE5] rounded-2xl">
          <span className="text-3xl mb-2">{seasonal.emoji}</span>
          <p className="text-sm font-medium text-[#1A1916]">Nenhum cliente com {seasonal.name} ainda</p>
          <p className="text-xs text-[#A8A59E] mt-1">Ative a campanha na página de cada cliente</p>
        </div>
      )}
    </div>
  )
