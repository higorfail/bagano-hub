...
import {
  ...
  slotsFor, isLate, todayISO, shiftISO, humanDate, logKey,
} from '@/lib/recurrings'
import {
...
/** Dentro da página de um cliente, `fixedClientId` tira o que ali é redundante:
 *  o filtro de cliente, o cabeçalho de grupo e o título da página. O resto é
 *  exatamente a mesma tela — é o que impede as duas de irem divergindo. */
...
export default function RecorrentesView({ fixedClientId }: Props) {
  const embedded = !!fixedClientId
  ...
  async function load() {
    ...
    // meio deixa a tela girando pra sempre, sem erro e sem saída.
    try {
      setFailed(false)
      const supabase = createClient()
      let recQuery = supabase.from('recurrings').select('*').order('position').order('created_at')
      if (fixedClientId) recQuery = recQuery.eq('client_id', fixedClientId)
  ...
  const paused = allFiltered.filter(r => !r.active).length
  const todayGroups = groupByClient(occurrences)
  const allGroups   = groupByClient(allFiltered.map(rec => ({ rec })))

  if (loading) return <div className={embedded ? 'text-sm text-[var(--color-text-muted)]' : 'p-6 text-sm text-[var(--color-text-muted)]'}>Carregando…</div>
  if (failed) return (
    <div className={`flex flex-col items-start gap-3 ${embedded ? '' : 'p-6'}`}>
  ...
  const rowsFor = (items: { rec: Recurring; slot: string; log: RecurringLog | null }[]) =>
  ...
  const toolbar = (
    <div className={`flex flex-col gap-2 xl:flex-row xl:items-center ${embedded ? '' : 'px-4 md:px-6 py-2.5 border-b border-[var(--color-border)]'}`}>
      {!embedded && (
        <div className="md:pr-3 md:mr-1 md:border-r border-[var(--color-border)] flex flex-col justify-center">
          ...
          <p className="text-[var(--color-text-muted)] text-[10px] mt-1">
            {view === 'hoje'
              ...
              : `${allFiltered.length} rotina${allFiltered.length === 1 ? '' : 's'} · ${paused} pausada${paused === 1 ? '' : 's'}`}
          </p>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-1">
        ...
        {/* Dentro de um cliente, filtrar por cliente não quer dizer nada. */}
        {!embedded && (
              ...
              <div className="absolute top-full left-0 mt-1.5 z-50 w-56 max-h-72 overflow-y-auto bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl shadow-xl p-1.5">
                ...
                {clients.map(c => (
                  <button key={c.id} onClick={() => setClientIds(s => { const n = new Set(s); n.has(c.id) ? n.delete(c.id) : n.add(c.id); return n })}
                    ...
                    {clientIds.has(c.id) && <Check size={13} className="text-[var(--color-accent)] flex-shrink-0" />}
                  </button>
                ))}
              </div>
            )}
  ...
  const hoje = (
    <>
      <div className={`flex items-center justify-center gap-1 ${embedded ? 'py-1' : 'px-4 py-2 border-b border-[var(--color-border)]'}`}>
        ...
        <button onClick={() => setIso(todayISO())}
          ...
          {humanDate(iso)}{embedded ? ` · ${pending} pendente${pending === 1 ? '' : 's'}` : ''}
        </button>
      ...
      {lateNow > 0 && (
        <div className={`flex items-center gap-2 py-2.5 text-xs font-bold text-white ${embedded ? 'px-3 rounded-xl' : 'px-4 md:px-6'}`} style={{ background: 'var(--ds-error-accent)' }}>
      ...
      {backlog.length > 0 && iso === todayISO() && (
        <div className={`flex items-start gap-2 text-xs px-3 py-2 rounded-xl border ${embedded ? '' : 'mx-4 md:mx-6 mt-3'}`}
      ...
      {occurrences.length === 0 ? (
        ...
        // Um cliente só: cabeçalho de grupo por cliente seria repetir o nome
        // que já está no topo da página.
            ...
            return (
              <div key={client.id} className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl">
                {/* Grudado no topo enquanto o grupo está na tela: com 8+
                    clientes, rolar fazia perder de vista de quem é a postagem. */}
                <div className="sticky top-0 z-10 flex items-center gap-2 px-3 md:px-4 py-2 border-b border-[var(--color-border)] bg-[var(--color-bg-card)] rounded-t-xl"
                  ...
                  <span className="text-[10px] font-bold ml-auto px-1.5 py-0.5 rounded"
                    style={tudo ? { background: 'var(--ds-success-bg)', color: 'var(--ds-success-text)' } : { color: 'var(--color-text-faint)' }}>
                    {feitos}/{items.length}
                  </span>
                </div>
                <div className="overflow-hidden rounded-b-xl">{rowsFor(items)}</div>
              </div>
            )
          })}
        </div>
  ...
  const todos = allFiltered.length === 0 ? (
    ...
    <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-3">
      {allFiltered.map(rec => <RecurringCard key={rec.id} rec={rec} logs={logsByRec[rec.id] || []} onEdit={r => setModal({ editing: r })} />)}
    </div>
  ) : (
  ...
  // Dentro do cliente a rolagem é da página, não daqui — um contêiner com
  // rolagem própria aninhado no outro dá aquela caixa que rola por dentro.
  if (embedded) {
    return (
      <div className="flex flex-col gap-3">
        {toolbar}
        {view === 'hoje' ? hoje : todos}
        {modalEl}
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      {toolbar}
      <div className="flex-1 overflow-y-auto">{view === 'hoje' ? hoje : todos}</div>
      {modalEl}
    </div>
  )
}
